...
def plateSplit(plateImagePath, imageSegmentsDownloadPath):
    ...
    # Optional : if u want to display the image
    ...
    while n < width - 1:
        ...
        if(white[n] > (1 - segmentation_spacing) * white_max):
            start = n
                           ...
                           segmentation_spacing)
            n = end
            if end - start > 5:
                character = thresholdedImage[1:height - 20, start:end]
                index += 1
                if(index != 1):
                    cv2.imwrite(imageSegmentsDownloadPath + '/' +
                                '{}'.format(index - 1) + '.jpg', character)


plateSplit("/home/pandakin/Desktop/12.png",
           "/home/pandakin/dev/ocr-moroccan-plates/platesplit")
